...
"  < vundle 插件配置 >
" -------------------------------------------------------------
set nocompatible              " be iMproved, required(关闭兼容模式)
filetype off                  " required(禁用文件类型侦)
...
if (g:airline_en)
	...
	set guifont=Droid_Sans_Mono_Slashed_for_Pow:h15:cANSI " 字体字号设置：h13代表字号
...
if (lookupfile_en)
	...
	nmap <c-f> :cclose<CR>:LookupFile<CR>
endif
...
if (g:code_complete)
    ...
    if (g:huayue_add)
        ...
        let g:template['c']['lgx'] = "log_info(\"".g:rs."...".g:re."=0x%x\", ".g:rs."...".g:re.");"
        let g:template['c']['lgd'] = "log_info(\"".g:rs."...".g:re."=%d\", ".g:rs."...".g:re.");"
        let g:template['c']['lgb'] = "log_info_hexdump(".g:rs."u8 *buf".g:re.", ".g:rs."u32 len".g:re.");"
...
function!  CscopeSync()
	...
	if(g:islinux)
        ...
        " else
            " !bash sync.sh&
            " terminal ++hidden ++close bash sync.sh
            !bash sync.sh
        " endif
	else
...
autocmd ShellCmdPost * silent call CscopeSyncLast()
" autocmd FileChangedShellPost * silent call CscopeSyncLast()
"---normal模式无动作后到达updatetime触发CursorHold事件
set updatetime=6000
" au CursorHold *.[ch] silent call CscopeSync()
"---常规模式下键入 sy 使能该工具 
nmap <silent>sy :call CscopeSync()<CR><CR><CR><CR>
...
function! Open_terminal()
    ...
    " silent only
    topleft terminal ++rows=11
endfunction

function! GNU_Makefile(target, param)
    " refresh variable, get make apps's name again
    let g:JL_MakeApps_name = system("grep '.PHONY' Makefile | grep -o 'app'")[:-2]
    let l:make_target = a:target
    let l:make_param = a:param
    let l:make_next = 'NONE'

    if l:make_target == ''
        let l:make_target = 'NONE'
    endif
    if l:make_param == ''
        let l:make_param = 'NONE'
    endif
    ...
    if 0
        ...
        set errorformat+=\
                        \%D%*\\a[%*\\d]:\ Entering\ directory\ %*[`']%f',
        if l:make_target == 'libs'
            if g:JL_MakeApps_name == 'app'
                exe "normal! :make clean ".l:make_param." && make libs ".l:make_param."\<cr>"
            else 
                exe "normal! :make clean_libs ".l:make_param." && make libs ".l:make_param."\<cr>"
            endif
        else
            exe "normal! :make clean ".l:make_param." && make ".g:JL_MakeApps_name." ".l:make_param."\<cr>"
        endif
    else
        if g:JL_MakeApps_name == ''
            let g:JL_MakeApps_name = 'NONE'
        endif

        wa
        " call term_sendkeys('zsh',"exit\<cr>")
        " exe "normal! \<c-w>w"
        " sleep 200ms
        sleep 200m
        if l:make_target == 'libs'
            if empty(glob("./MakeLibsIsRuning"))
                if !empty(glob("./MakeAppsIsRuning"))
                    " exe "normal! \<c-w>w"
                    return
                endif
                " exe "normal :!touch MakeLibsIsRuning\<cr>"
                " terminal ++hidden ++close touch MakeLibsIsRuning
                call system("touch MakeLibsIsRuning")
                call Open_terminal()
                if (g:islinux)
                    " \<cr> usage <:help term_sendkeys> for detail
                    let l:make_next = g:JL_MakeApps_name
                    call term_sendkeys('',"bash MakeErrorOutput.sh ".l:make_target." ".l:make_param." ".l:make_next." && exit\<cr>")
                else
                    call term_sendkeys('',"call MakeErrorOutput.bat libs ".a:param."\<cr>")
                endif
            endif
            " jump out of the terminal window to continue editing file
            exe "normal! \<c-w>w"
        else
            if empty(glob("./MakeAppsIsRuning"))
                " exe "normal :!touch MakeAppsIsRuning\<cr>"
                " terminal ++hidden ++close touch MakeAppsIsRuning
                call system("touch MakeAppsIsRuning")
                if !empty(glob("./MakeLibsIsRuning"))
                    " exe "normal! \<c-w>w"
                    return
                endif
                call Open_terminal()
                if (g:islinux)
                    let l:make_target = g:JL_MakeApps_name
                    call term_sendkeys('',"bash MakeErrorOutput.sh ".l:make_target." ".l:make_param." ".l:make_next." && exit\<cr>")
                else
                    call term_sendkeys('',"call MakeErrorOutput.bat ".l:make_param."\<cr>")
                endif
            endif
            " jump out of the terminal window to continue editing file
            exe "normal! \<c-w>w"
...
function! GNU_Makefile_assign(target)
        wa
        " call term_sendkeys('zsh',"exit\<cr>")
        " exe "normal! \<c-w>w"
        " sleep 200ms
        sleep 200m
        if a:target == 'btctrler'
            if empty(glob("./MakeLibsIsRuning"))
                call system("touch MakeLibsIsRuning")
                call Open_terminal()
                " \<cr> usage <:help term_sendkeys> for detail
                call term_sendkeys('',"bash MakeErrorOutput.sh btctrler && exit\<cr>")
            endif
            " jump out of the terminal window to continue editing file
            exe "normal! \<c-w>w"
        endif
...
" ---for Makefile build
" nmap <F6> :call GNU_Makefile_assign('btctrler')<CR><CR>
nmap <F6> :call GNU_Makefile('libs', '-j')<CR><CR>
nmap <F7> :call GNU_Makefile('apps', '-j')<CR><CR>
...
" uart打印快捷键
imap pu<Enter> puts("\r\n");<Esc>F\F\i
imap log<Enter> log_info("");<Esc>F"i
imap pc<Enter> putchar('');<Esc>F'i
cmap putchar putchar('')<Left><Left>
" imap pfx<Enter> printf("=0x%x\n", );<Esc>F=i
" imap pfd<Enter> printf("=%d\n", );<Esc>F=i
imap pff<Enter> printf("\n--func=%s\r\n", __FUNCTION__);<Esc>
imap lgf<Enter> log_info("--func=%s", __FUNCTION__);<Esc>
imap lgl<Enter> log_info("--line=%d", __LINE__);<Esc>
" bit操作快捷键
...
" imap (<Enter> ()<Esc>i
" endif注释 
imap #ef<Enter>  #endif /*  */<Left><Left><Left><Esc>
...
function! COPY_sync()
    if (g:islinux)
        if empty(glob("./sync.sh"))
            !cp ~/git_hub/LINUX_TOOL/vim/.vim/tools/sync.sh ./
        endif
    " else
    "     if empty(glob("./sync.bat"))
    "         !copy $vim\vimfiles\tools\sync.bat .\
    "     endif
    endif
endfunction
" map <silent><F3> :call COPY_sync()<CR><CR><CR><CR>
...
" ---常规模式下 编辑svn config文件(仅适用于linux)
" nmap <Leader>ec      :e ~/.subversion/config<CR>

" ---常规模式下 vim diff 快速合并下一个不同处
" nmap <F8>           ]cdp

" ---常规模式下 vim嵌套gdb调试
if(g:islinux)
    function! VIM_gdb()
        " :lefta vert terminal ++close ++rows=52 ++cols=55 gdb -q ./apps/port/br18/post_build/sdk.elf -ex "target remote proxy:br18_debug:9872"
        " :lefta vert terminal ++close ++cols=85 /opt/gdb ./cpu/bd29/tools/sdk.elf -ex "target remote proxy:bd29_sdk_jtag:9872"
        :lefta vert terminal ++close ++cols=85 /opt/utils/gdb-7.12 ./cpu/bd29/tools/sdk.elf -ex "target remote proxy:bd29_sdk_jtag:9872"
    endfunction
    " nmap <F8>   :call VIM_gdb()<CR>
endif

" --- normal mode下vim8.1 gdb7.12以上内置调试
if (g:islinux)

" JL SoC select (vaild global)
let g:JL_SoC = 'AUTO'

" 显示器方向：自动识别(auto) ; 纵向(vertical) ; 横向(lateral) (vaild global)
let g:gdb_display_direction = 'AUTO'
" let g:gdb_display_direction = 'vertical'
" let g:gdb_display_direction = 'lateral'

" 仪表盘显示模式：'MODE_0'; 'MODE_1'; 'MODE_2'; 'MODE_GNOME'
let g:dashboard_display_mode = 'MODE_0'

" gdb path
let g:termdebugger = "/opt/utils/gdb-7.12"

function! Reset_chip()
    call TermDebugSendCommand("reset")
    " sleep 100m
...
function! Enter_BuiltIn_gdb()
    " refresh variable, get SoC name again
    let g:JL_SoC = system("grep 'export SoC' Makefile | sed -e '/#export/d' -e '/# export/d' -e 's/export SoC?=//g'")[:-2]
    " echo "JL_SoC: ".g:JL_SoC
    if !empty(glob("./.GdbIsExist"))
        echoerr "   !!! Gdb already exist !!!"
        return
    else 
        " gdb can not open again now
        call system("touch ./.GdbIsExist")
    endif
    " close some plug buffer
    silent NERDTreeClose
    silent TlistClose
    cclose
    " Make the current window the only one on the screen.  All other windows are closed
    silent only
    " add built-in gdb pack
    packadd termdebug
    " TermdebugCommand ./cpu/bd29/tools/sdk.elf
    " call TermDebugSendCommand('target remote proxy:bd29_sdk_jtag:9872')
    Termdebug -q
    call TermDebugSendCommand("file ./cpu/".g:JL_SoC."/tools/sdk.elf")
    call TermDebugSendCommand('target remote proxy:'.g:JL_SoC.'_sdk_jtag:9872')
    call TermDebugSendCommand('source ./.dbg_breakpoints')
    " 屏幕显示处理
    exe "normal! \<c-w>w"
    exe "normal! \<c-w>:"
    q!

    if (g:dashboard_display_mode != 'MODE_GNOME')
        if (g:gdb_display_direction == 'AUTO')
            if (&lines > 75)
                resize 25
                " echo "lines:"&lines.", 当作\" 竖屏 \"处理"
            else
                exe "normal! \<c-w>H"
                vertical resize 70
                " echo "lines:"&lines.", 当作\" 横屏 \"处理"
            endif
        else
            if (g:gdb_display_direction == 'vertical')
                resize 25
            else
                exe "normal! \<c-w>L"
                vertical resize 70
    ...
    if (g:dashboard_display_mode == 'MODE_0')
        exe "normal! \<c-w>w"
        term ++rows=43
        call term_sendkeys('zsh',"tty > .Get_tty\<cr>")
        while empty(glob(".Get_tty"))
        endwhile
        let s:main_tty = system("cat .Get_tty")[:-2]
        call TermDebugSendCommand("dashboard -output ".s:main_tty)
        call TermDebugSendCommand("!clear")
        exe "normal! \<c-w>w"
        exe "normal! \<c-w>w"
    elseif (g:dashboard_display_mode == 'MODE_1')
        exe "normal! \<c-w>w"
        term ++rows=43
        call term_sendkeys('zsh',"tty > .Get_tty\<cr>")
        while empty(glob(".Get_tty"))
        ...
        vertical botright term ++cols=17
        call term_sendkeys('zsh (1)',"rm .Get_tty\<cr>")
        while !empty(glob(".Get_tty"))
        endwhile
        call term_sendkeys('zsh (1)',"tty > .Get_tty\<cr>")
        while empty(glob(".Get_tty"))
        endwhile
        let s:reg_tty = system("cat .Get_tty")[:-2]
        call TermDebugSendCommand("dashboard registers -output ".s:reg_tty)

        exe "normal! \<c-w>w"
    elseif (g:dashboard_display_mode == 'MODE_2')
        exe "normal! \<c-w>w"
        term ++rows=33
        call term_sendkeys('zsh',"tty > .Get_tty\<cr>")
        while empty(glob(".Get_tty"))
        ...
        while empty(glob(".Get_tty"))
        endwhile
        let s:asm_tty = system("cat .Get_tty")[:-2]
        call TermDebugSendCommand("dashboard assembly -output ".s:asm_tty)

        vertical belowright term ++cols=80
        call term_sendkeys('zsh (2)',"rm .Get_tty\<cr>")
        while !empty(glob(".Get_tty"))
        endwhile
        call term_sendkeys('zsh (2)',"tty > .Get_tty\<cr>")
        while empty(glob(".Get_tty"))
        endwhile
        let s:source_tty = system("cat .Get_tty")[:-2]
        call TermDebugSendCommand("dashboard source -output ".s:source_tty)

        vertical botright term ++cols=17
        call term_sendkeys('zsh (3)',"rm .Get_tty\<cr>")
        while !empty(glob(".Get_tty"))
        endwhile
        call term_sendkeys('zsh (3)',"tty > .Get_tty\<cr>")
        while empty(glob(".Get_tty"))
        endwhile
        let s:reg_tty = system("cat .Get_tty")[:-2]
        call TermDebugSendCommand("dashboard registers -output ".s:reg_tty)
        ...
        exe "normal! \<c-w>w"
    elseif (g:dashboard_display_mode == 'MODE_GNOME')
        exe "normal! \<c-w>L"
        vertical resize 150
        call system("echo 'tty>.Get_tty' > .gdb_script.sh")
        call system("gnome-terminal -x bash .gdb_script.sh")
        while empty(glob(".Get_tty"))
        endwhile
        call system("gnome-terminal")
        call system("rm .gdb_script.sh")
        let s:GnomeTerminal_tty = system("cat .Get_tty")[:-2]
        call TermDebugSendCommand("dashboard -output ".s:GnomeTerminal_tty)
        call TermDebugSendCommand("!clear")
    endif

    " gdb cmd map, detail type :help Termdebug
    nmap ;b             :Break<CR>
    nmap ;d             :Clear<CR>
    nmap ;c             :Continue<CR>
    nmap ;f             :Finish<CR>
    nmap ;n             :Over<CR>
    nmap ;r             :call Reset_chip()<CR>
    nmap <Leader>ek     :Evaluate <C-R>=expand("<cword>")<cr><cr>
    nmap <Leader>gd     :call TermDebugSendCommand("display/x ".expand("<cword>"))<CR>
    nmap <Leader>gp     :call TermDebugSendCommand("p/x ".expand("<cword>"))<CR>
    nmap ;q             :call Exit_gdb()<CR>
    " keep normal vim "K" map
    nmap K              5k
endfunction

function! Exit_gdb()
    call term_sendkeys('zsh',"exit\<cr>")
    call term_sendkeys('zsh (1)',"exit\<cr>")
    call term_sendkeys('zsh (2)',"exit\<cr>")
    call term_sendkeys('zsh (3)',"exit\<cr>")
    call TermDebugSendCommand('qq')
    " unmap gdb map, resume noraml vim map
    nunmap ;b
    nunmap ;d
    nunmap ;c
    nunmap ;f
    ...
    " keep normal vim "K" map
    nmap K 5k
    " gdb can open again now
    call system("rm ./.GdbIsExist")
endfunction

" edit .gdbinit
nmap <Leader>eg :e ~/.gdbinit<CR>
" enter vim8.1 built-in gdb
nmap <F8>       :call Enter_BuiltIn_gdb()<CR>
...
" ---利用终端find和grep定位字符串到quickfix
function! Jump_find_result()
    " cf FindString.result
    " botright copen 7
    if !empty(glob("./FindString.result"))
        cf FindString.result
        botright copen 7
    else
        echohl "!!! Can not find target !!!"
        cclose
    endif
endfunction

if (g:islinux)
    nmap <Leader>fa :!bash FindString.sh <C-R>=expand("<cword>")<CR><CR>:call Jump_find_result()<CR>
    vmap <Leader>fa <C-C>:!bash FindString.sh <S-Insert><CR><CR>:call Jump_find_result()<CR>

	nmap <Leader>ef :e ~/.vim/tools/FindString.sh<CR>
endif
...
" ---高亮 #if 0 部分
function! Hi_IF0()
    syntax region My_highlight start=/#if 0/ end=/#e/
    highlight My_highlight ctermfg=gray guifg=#00FFFF
endfunction
autocmd BufReadPost *.c call Hi_IF0()
autocmd BufEnter *.c call Hi_IF0()

" ---快速打开lst文件
autocmd BufEnter *.lst silent exe "normal! \<c-c>"
" autocmd BufReadPre *.lst silent exe "normal! \<c-c>"
" autocmd BufReadPost *.lst silent exe "normal! \<c-c>"


" ---配置 quickfix current line color
" hi QuickFixLine ctermfg=84 ctermbg=NONE cterm=NONE guifg=#50fa7b guibg=NONE gui=NONE
